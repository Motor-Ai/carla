...
def closest_wp_idx(ego_state, fpath, f_idx, w_size=10):
    ...
    w_size = w_size if w_size <= len(fpath.t) - 5 - f_idx else len(fpath.t) - 5 - f_idx
...
def update_frenet_coordinate(fpath, loc):
    ...
    min_e = float('inf')
    min_idx = -1
    for i in range(min(len(fpath.t), len(fpath.x), len(fpath.y))):
    ...
    if min_idx <= len(fpath.t) - 2:
        min_idx += 1  # +2 because if next wp gets too close to the ego, lat controller oscillates
    ...
    try:
        ...
        print(len(fpath.t), len(fpath.x), len(fpath.s))
...
class FrenetPlanner:
    def __init__(self):
        ...
        # Parameters
        self.MAX_SPEED = 200.0 / 3.6  # maximum speed [m/s]
    ...
    def closest_pt_in_path(self, ego_state, fpath, f_idx, w_size=10):
        """
        given the ego_state and frenet_path this function returns the closest point in the path
        """
        f_state = self.estimate_frenet_state(ego_state, f_idx)

        min_dist = 300  # in meters (Max 100km/h /3.6) * 2 sn
        #closest point in f_state.s and fpath.s

        idx = np.argmin(np.abs(fpath.s - f_state[0]))
        return idx
    ...
    def get_curvature_arr(self, fplist):
        """
        Calculate curvature of the input path
        """
        for path in fplist:
            x_arr,y_arr = path.x, path.y
            if len(x_arr) > 5:
                # Single differential
    ...
    def frenet_optimal_planning(self, f_state, ego_state = None, change_lane=0, target_speed=30 / 3.6):
        """
        ...
        fplist = self.get_curvature_arr(fplist)
        ...
        # plt.scatter(self.path.y, self.path.x)
    ...
    def reset(self, s, d, df_n=0, Tf=4, Vf_n=0, optimal_path=True):
        ...
        if optimal_path:
            ...
            self.main_path = self.generate_single_frenet_path(f_state, df=df, Tf=Tf, Vf=Vf)
            self.path = copy.deepcopy(self.main_path)
    ...
    def run_step(self, ego_state, idx, change_lane=0, target_speed=30 / 3.6):
        ...
        # print("f_state1:{}".format(f_state))
        f_state = self.estimate_frenet_state(ego_state, idx)

        # print(ego_state)
        # print("f_state2:{}".format(f_state))
        # Frenet motion planning
        best_path_idx, fplist = self.frenet_optimal_planning(f_state,ego_state=ego_state, change_lane=change_lane,
                                                             target_speed=target_speed)
        # print("best_path_idx:{}".format(best_path_idx))
        self.path = fplist[best_path_idx]
    ...
    def run_step_single_path(self, ego_state, idx, df_n=0, Tf=4, Vf_n=0):
        ...
        f_state  = update_frenet_coordinate(self.main_path, ego_state[:2])
        # print(f_state)
        # f_state = self.estimate_frenet_state(ego_state, idx)
