...
Q  = np.diag([0.05, 0.05, 0.0, 0.1, 0.01, 5.0])  # state cost matrix
Qf = np.diag([0.05, 0.05, 0.0, 0.1])  # state cost matrix
...
def calc_ref_trajectory(state, cx, cy, cyaw, ck, sp, dl):
    ...
    #state = delay_corrected_state(state, 0.2)

    ind, lat_dist = calc_nearest_index(state, cx, cy, cyaw)
...
def iterative_linear_mpc_control(self, xref, x0, dref, oa, od, ego_vel, lane_morph, target_speed, throttle_value_change):
    ...
    for i in range(T):                                       ## MAX_ITER can be replaced by T
        print('X0:  ' + str(np.round(np.degrees(x0[4]))))
        print('Yaw_ref' + str(np.round(np.degrees(xref[3]))))
        
        xbar = predict_motion(self, x0, oa, od, xref)

        print('Yaw_pred' + str(np.round(np.degrees(xbar[3]))))
        ...
        if abs(xref_corr[3][0] - xbar[3][0]) > np.radians(90.0):
            ...
            print('Yaw_ref corr' + str(np.round(np.degrees(xref_corr[3]))))


        poa, pod = oa[:], od[:]
        _, od, ox, oy, oyaw, ov, weight = linear_mpc_control(self, xref_corr, xbar, x0, dref, ego_vel, lane_morph, target_speed, throttle_value_change)

        print('ODelta' + str(np.round(np.degrees(od))))
...
def linear_mpc_control(self, xref, xbar, x0, dref, ego_vel, lane_morph, target_speed, throttle_value_change):
    ...
    #Q  = np.diag([0.1, 0.1, 0.0, 1.0, 0.01, 5.0])  # state cost matrix
    #Qf = np.diag([0.1, 0.1, 0.0, 1.0])  # state cost matrix
    ...
    if lane_morph in ['turn', 'u-turn', 'high_curvature']:
        ...
        print('highspeed ' + str(np.round(target_speed-ego_vel,2)))
        #Q_steer_rate_weight_pow = np.round(np.clip((ego_vel)*1.8, 5.0,20.0),1)
        # Q_steer_rate_weight_pow = np.round(np.clip((target_speed)*1.8, 5.0,20.0),1)
        Q[5,5] = Q_steer_rate_weight_pow
        Q[3,3] = 1.0
        Qf[3,3] = 1.0
        ...
        Q_steer_rate_weight_pow = 5.0
        print('normal ' + str(np.round(target_speed-ego_vel,2)))
        Q[5,5] = Q_steer_rate_weight_pow
        Q[3,3] = 1.0
        Qf[3,3] = 1.0
        ...
        Q_steer_rate_weight_pow = 2.0
        print('other ' + str(np.round(target_speed-ego_vel,2)))
        Q[5,5] = Q_steer_rate_weight_pow
        Q[3,3] = 1.0
        Qf[3,3] = 1.0
...
def predict_motion(self, x0, oa, od, xref):
    ...
    state = State(x=x0[0], y=x0[1], yaw=x0[3], v=x0[2], delta=x0[4])
    for (ai, di, i) in zip(oa, od, range(1, T + 1)):       
        state = update_state(state, 0.0, di, DT, True)        
        xbar[0, i] = state.x
        xbar[1, i] = state.y
        xbar[2, i] = state.v
        xbar[3, i] = state.yaw
    ...
    # if abs(xref[3][0] - xbar[3][0]) > np.radians(90.0):
    #     if xref[3][0] > 0.0:
    #         xbar[3][0] = xbar[3][0] + np.radians(360.0)
    #     elif xref[3][0] < 0.0:
    #         xbar[3][0] = xbar[3][0] - np.radians(360.0)
    #     xbar[3] = smooth_yaw(xbar[3])

    # print('Yaw_pred after' + str(np.round(np.degrees(xbar[3]))))
    
    #xbar[3] = normalize_yaw_pred(xref[3], xbar[3])
    #yaw_pred = normalize_yaw_pred(xref[3], xbar[3])
    #print('Yaw_pred 2' + str(np.round(np.degrees(yaw_pred))))
